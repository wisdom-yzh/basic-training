...
class Solution {
  ...
  vector<vector<int>> quad(vector<vector<int>> &grid, int row, int col,
                           int size) {
    ...
    for (int count = 0; count < size; count++) {
      ...
      if (size == 1) {
        ...
        res.push_back(vector<int>(tmp.begin() + col, tmp.begin() + col + size));
      }
    }
    return res;
  }
...
public:
  Node *construct(vector<vector<int>> &grid) {
    ...
    return new Node(0, false, construct(topLeft), construct(topRight),
